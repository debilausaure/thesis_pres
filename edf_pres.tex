...
\begin{frame}[fragile]
	\frametitle{Schedulability hypothesis}

	Given any two moments $t, t'$, let $\Gamma_{t, t'}$ be the set of jobs $j$ to schedule in the interval $[t, t']$.\\
	If the sum of the budget $c_j$ of the jobs in that set is less than $t' - t$, then the jobs are \emph{schedulable}.

	\begin{gather}
		\forall ~t, t'. ~t < t' \implies
		\sum_{j \in \Gamma_{t, t'}}c_j \leq t' - t
	\end{gather}
\end{frame}
