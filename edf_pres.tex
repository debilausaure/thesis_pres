...
\begin{frame}[fragile]
	...
	The election function is proved \emph{correct} under reasonable assumptions :\\
	schedulable job sets are scheduled such that no job misses its deadline.
\end{frame}
...
\begin{frame}[fragile]
	...
	\[
		\text{schedulable} \iff \forall ~t, t'. ~t < t' \implies \sum_{j \in \Gamma_{t, t'}}c_j \leq t' - t
	\]
	\end{definition}
\end{frame}

\begin{frame}[fragile]
	\frametitle{Additional "well-formedness" assumptions}
...
\begin{frame}[fragile]
	...
	Implement an idealised, functional (non-translatable) election function that acts like the final one.\\[0.5cm]
	...
	\[
		...
		\texttt{functional\_scheduler}(t) = (o, s) \implies
...
\begin{frame}[fragile]
	...
	Implement the final, compilable to C, imperative election function \emph{that relies on a chosen set of primitives}.\\[0.5cm]
	...
	\begin{block}{Actual election function has same effects as functional}
		...
		\begin{gather*}
			...
			\texttt{scheduler}~(t)\\
			\{
			    ~\lambda~o~s'. ~\texttt{functional\_scheduler}~(t) = (o,s')~
			\}
		\end{gather*}
	\end{block}
