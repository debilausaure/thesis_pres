\begin{tikzpicture}[>=triangle 45,font=\sffamily, every text node part/.style={align=center}, scale=0.55, every node/.style={transform shape}]
	...
	\onslide<1> {
		...
		\draw[<-, dashed] (child1.west) to [out=135, in=225] node [midway, left] {Demande d'ouverture de fichier} (gchild.west);
	}
	\onslide<2> {	
		\node[draw, thick, minimum width=4cm, minimum height=2cm] (pip) at (0, -3) {Pip};
		...
		\node[opacity=0.3, draw, thick, minimum width=4cm, minimum height=2cm] (root) at (0, 0) {Multiplexer};
		\node[draw, thick, fill=pinkcolor, minimum width=4cm, minimum height=2cm] (child1) at (-5, 3) {};
		\node[draw, thick, fill=white, minimum width=3.6cm, minimum height=2cm] at (child1) {Linux};
		\node[opacity=0.3, draw, thick, minimum width=4cm, minimum height=2cm] (child2) at (5, 3) {FreeRTOS};
	
		\node[draw, thick, fill=bluecolor, minimum width=4cm, minimum height=2cm] (gchild) at (-5, 6) {};
		\node[draw, thick, fill=white, minimum width=3.6cm, minimum height=2cm] at (gchild) {Shell};
		...
		\draw[<-, dashed, color=black!30] (child1.west) to [out=135, in=225] node [midway, left, color=black!30] {Demande d'ouverture de fichier} (gchild.west);
		\draw[->, dashed] (child1.east) to [out=45 , in=315] node [midway, right] {Le fichier a été ouvert,\\reprise de l'exécution} (gchild.east);
	}
\end{tikzpicture}
